# This function is written to get list of EC2 instances using filter by tags as "env": "dev" and that are in 'stopped' state and start them.
# Here, first find all the instance id as per Tags and once a Python List of Instance ids is created, this can be passed to ec2.start_instances(InstanceIds=InstanceIdsList)
...
for instances in reservations_list:
	...
	if not instances['Instances'][0]['Tags']:
		print("Tags Key is not available for this instance")
	else:
		...
		if not tags_list:
			print("Tags not available for this instance")
		else:
			for tags in tags_list:
				print(type(tags))
				if tags['Key'] == 'env' and tags['Value'] == 'dev':
				if tags['Key'] == 'Name' and tags['Value'] == 'webservers':
...
'''
{
	'Reservations': [{
		...
		'Instances': [{
			...
			'Hypervisor': 'xen',
			'IamInstanceProfile': {
				'Arn': 'arn:aws:iam::082123705555:instance-profile/EC2-AWS-CICD-Roile',
